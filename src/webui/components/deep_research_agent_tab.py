...
async def run_deep_research(
    ...
    if not task_topic:
        gr.Warning("Please enter your research task.")
    ...
    with gr.Group():
        ...
        with gr.Row():
            resume_task_id = gr.Textbox(
                ...
                value="",
                info="Task ID to resume from (leave blank to start a new task)",
                interactive=True,
            )
            parallel_num = gr.Number(
                label="Parallel Agent Number",
                value=1,
                precision=0,
                info="Number of parallel agents to run research tasks",
                interactive=True,
            )
            max_query = gr.Textbox(
                label="Research Save Path",
                value="./tmp/deep_research",
                info="Path to save research task results",
                interactive=True,
            )
    with gr.Row():
        stop_button = gr.Button("⏹️ Stop", variant="stop", scale=2)
        start_button = gr.Button("▶️ Run", variant="primary", scale=2)
    with gr.Group():
